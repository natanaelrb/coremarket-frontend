...
export function LotesVencidos({ lotes, onVerTodos }) {
  return (
    <WidgetCard title="Lotes vencidos" actionLabel="Ver todos" onAction={onVerTodos}>
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-xs text-gray-400 dark:text-gray-500">
                Lote: {lote.lote} · Qtd: {formatNumber(lote.quantidade)}
              </p>
            </div>
            <div className="text-right">
              <p className="text-xs text-gray-500 dark:text-gray-400">{formatDate(lote.validade)}</p>
              <p className="text-xs font-medium text-red-500">-{lote.diasVencido} dias</p>
            </div>
          </li>
        ))}
      </ul>
