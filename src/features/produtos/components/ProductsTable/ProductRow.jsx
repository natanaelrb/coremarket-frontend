...
export function ProductRow({ produto, isSelected, isVisible, onToggleSelect, onOpenDetail, rowActions }) {
  ...
  return (
    <tr className="group border-b border-gray-50 transition-colors duration-100 last:border-0 hover:bg-violet-50/40 dark:border-gray-800/60 dark:hover:bg-white/[0.03]">
      <td className="w-10 px-4 py-3">
        <Checkbox checked={isSelected} onChange={() => onToggleSelect(produto.id)} ariaLabel={`Selecionar ${produto.nome}`} />
      </td>

      {isVisible('imagem') && (
        <td className="px-2 py-3">
          <button onClick={() => onOpenDetail(produto)} className="block">
            <ProductImage emoji={produto.imagemEmoji} color={produto.imagemCor} />
          </button>
        </td>
      )}
      ...
      {isVisible('codigoBarras') && (
        <td className="px-2 py-3 text-sm text-gray-500 dark:text-gray-400">{produto.codigoBarras}</td>
      )}
      ...
      {isVisible('marca') && (
        <td className="px-2 py-3 text-sm text-gray-600 dark:text-gray-300">{produto.marca}</td>
      )}
      ...
      {isVisible('estoque') && (
        <td className={`px-2 py-3 text-sm font-semibold ${estoqueColorClass}`}>{formatNumber(produto.estoque)}</td>
      )}

      {isVisible('estoqueMinimo') && (
        <td className="px-2 py-3 text-sm text-gray-500 dark:text-gray-400">{formatNumber(produto.estoqueMinimo)}</td>
      )}
