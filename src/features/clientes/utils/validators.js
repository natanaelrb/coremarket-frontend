 ...
 */
export function validateRegistrarPagamento(form, contasEmAberto = []) {
  const errors = {};

  if (!form.contaId) {
    errors.contaId = "Selecione a conta.";
  }
  ...
  const conta = contasEmAberto.find(
    (item) => String(item.id) === String(form.contaId)
  );

  if (conta && valor > Number(conta.saldo)) {
    errors.valor = "O pagamento não pode ser maior que o saldo da conta.";
  }
